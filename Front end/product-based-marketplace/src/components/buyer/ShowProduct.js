...
const Product = () => {
    ...
    const url = `http://127.0.0.1:8000/api/buyer/showProduct/${user[0].id}`;
    ...
    async function Search() {
        let result = await fetch(`http://127.0.0.1:8000/api/buyer/showProduct/search/${user[0].id}/` + key)
    ...
    async function ShowAll() {
        let result = await fetch(`http://127.0.0.1:8000/api/buyer/showProduct/${user[0].id}`)
        result = await result.json();
        setProductList(result);
    }
